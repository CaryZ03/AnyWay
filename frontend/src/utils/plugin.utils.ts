...
export function pluginToForm(plugin: Plugin): PluginForm {
  // 如果 openapiSpec 已经是对象，直接使用；否则解析字符串；如果不存在，使用默认值
  let openapiSpec: OpenAPISpec
  if (plugin.openapiSpec !== undefined && plugin.openapiSpec !== null) {
    if (typeof plugin.openapiSpec === 'string') {
      openapiSpec = JSON.parse(plugin.openapiSpec || '{}') as OpenAPISpec
    } else {
      openapiSpec = plugin.openapiSpec
    }
  } else {
    openapiSpec = createDefaultOpenAPISpec()
  }

  // 如果 config 已经是对象，直接使用；否则解析字符串；如果不存在，使用默认值
  let config: PluginConfig
  if (plugin.config !== undefined && plugin.config !== null) {
    if (typeof plugin.config === 'string') {
      config = JSON.parse(plugin.config || '{}') as PluginConfig
    } else {
